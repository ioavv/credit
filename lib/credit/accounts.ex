defmodule Credit.Accounts do
  require Amnesia
  require Amnesia.Helper
  require Exquisite
  require Database.Account

  alias Databse.Account

  def create_account(username, starting_balance) do
    Amnesia.transaction do
      %Account{username: username, balance: starting_balance}
      |> Account.write()
    end
  end
  ...
  def transfer_funds(source_account_id, destination_account_id, amount) do
    Amnesia.transaction do
      ...
      case accounts do
        {%Account{} = source_amount, %Account{} = distination_account} ->
